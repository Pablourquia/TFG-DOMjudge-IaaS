...
class DOMJudgeService
{
    ...
    public function getTwigDataForProblemsAction(int $teamId, StatisticsService $statistics): array
    {
        ...
        $question_contest = $this->em->getRepository(QuestionContest::class)->findOneBy(['contestName' => $contest->getName()]);
        $questions_ids = [];
        if ($question_contest) {
            $questions_ids = $question_contest->getQuestions();
        }
        $questions = [];
        foreach ($questions_ids as $question_id) {
            $question = $this->em->getRepository(Question::class)->find($question_id);
            $questions[] = $question;
        }
        $data = [
            'problems' => $problems,
